...
# forest.conf.mat.prob = svm.conf.mat
...
plot_conf_mat(svm.conf.mat.prob$table)
plot_conf_mat(tree.conf.mat.prob$table)
plot_conf_mat(forest.conf.mat.prob$table)
...
acc$class <- factor(acc$class, levels=rev(levels(plt$class)))
  
# ggplot original code from: https://stackoverflow.com/a/64539733  
ggplot(acc, aes(class, svm, fill = Freq)) +
        geom_tile() + geom_text(aes(label=Freq)) +
        scale_fill_gradient(low="white", high="#85bdde") +
        labs(x = "Reference",y = "Prediction") +
        scale_x_discrete(labels=c("GALAXY","QSO","STAR")) +
