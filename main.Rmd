...
```{r}
dataset.train.pca <- PCA(dataset.train, scale.unit = FALSE, ncp = 8, graph = FALSE)
...
```{r}
dataset.train.pca = PCA(dataset.train, scale.unit = FALSE, ncp = 4, graph = FALSE)
...
svm.radial = svm(
 target ~ Dim.1 + Dim.2 + Dim.3 + Dim.4,
 data        = dataset.train,
 kernel      = 'radial',
 probability = FALSE,
 cost        = 1
